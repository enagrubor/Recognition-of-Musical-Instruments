...
#Ovim se svaka kolona (svaki MFCC koeficijent ili njegova std vrednost) skalira tako da ima:
...
# Regularizovana kovarijaciona matrica (sprečava singularnost)
V = np.cov(X.T) + 1e-6 * np.eye(X.shape[1])

VI = inv(V)  # Inverzna kovarijaciona matrica
...
#plt.title("KNN - Originalni MFCC (Mahalanobis distanca)", fontsize=14, fontweight = 'bold')
plt.title("KNN - Originalni MFCC (Mahalanobis distanca, oteženjena)", fontsize=13, fontweight = 'bold')
plt.show()
...
# Izračunaj kovarijacionu matricu za LDA podatke
...
print("KNN tačnost (LDA - Mahalanobis):", round(acc_lda, 3))
...
#plt.title("KNN - LDA redukcija (Mahalanobis distanca)", fontsize=14, fontweight = 'bold')
plt.title("KNN - LDA redukcija (Mahalanobis distanca, oteženjena)", fontsize=13, fontweight = 'bold')
plt.show()
...
# Izračunaj kovarijacionu matricu za LFDA podatke
...
print("KNN tačnost (LFDA - Mahalanobis):", round(acc_lfda, 3))
...
#plt.title("KNN - LFDA redukcija (Mahalanobis distanca)", fontsize=14, fontweight = 'bold')
plt.title("KNN - LFDA redukcija (Mahalanobis distanca, oteženjena)", fontsize=13, fontweight = 'bold')
plt.show()
...
# --- Kov. matrice za svaku varijantu ---
V = np.cov(X.T) + 1e-6 * np.eye(X.shape[1])
VI = inv(V)
...
for k in k_values:
    ...
    knn_raw = KNeighborsClassifier(
        ...
        metric='mahalanobis',
        metric_params={'VI': VI},
        weights='distance'     # otežena verzija
    )
    knn_raw.fit(X, y)
